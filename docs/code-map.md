...
本文件索引官方登录、普通第三方和 Sub2API Responses API 切换工具。
...
-> 修改 model/model_provider/model_providers.custom 或 model_providers.sub2api
...
面向用户的 WinForms 单文件程序。支持选择 Codex 根目录、填写 URL/Key/模型、查看当前状态、切换官方/普通第三方/Sub2API 模式、恢复最近备份，以及在模型配置损坏时一键重建官方基础配置。普通第三方与 Sub2API 使用独立的 DPAPI 凭据和输入草稿；Sub2API provider 固定设置 `supports_websockets = false`。
...
“一键恢复基础配置”会先备份 `config.toml`，补回顶层 `model_provider`/`model`，移除损坏的 `model_providers.custom` 和 `model_providers.sub2api` 段，并保留 MCP、插件、沙箱等其他配置；切换时会重新生成目标 provider。

`work/CodexApiSwitcher.cs`

GUI 与切换核心源码。EXE 同时提供 `--emit-token --profile custom|sub2api` 凭据助手模式，供 Codex 获取对应 DPAPI 解密 Key。SQLite 操作通过 P/Invoke 调用 Windows 自带 `winsqlite3.dll`，不启动 Python 或外部 SQLite 程序。
...
验证 EXE 在 PATH 不含 Python 时仍可完成官方、普通第三方和 Sub2API 三模式切换，同时同步 SQLite 与 JSONL provider 元数据，确认会话正文不变，并覆盖独立 DPAPI 凭据、Sub2API WebSocket 关闭、真实 Codex CLI 配置解析、侧栏修复、基础配置重建、Base URL `/v1` 规范化、自动备份、TOML 解析和无关配置保留。
